...
//  }
        ...
        function showListDetalleMaterial($type){
            if($type == "DISPONIBLE"){
                ...
                $sql="SELECT p.idEstudiante,dt.idDetalleMaterial,dt.codigo,p.codePecosa,dt.status,e.firstName,e.LastName,grado,section FROM prestamo p 
                inner join detallematerial dt on p.idDetalleMaterial=dt.idDetalleMaterial inner join 
                estudiante e on p.idEstudiante=e.idEstudiante";
            }else{
                $sql="SELECT dtv.idDevolucion,dt.codigo,e.firstName,e.LastName,e.grado,e.section,dtv.fechaHoraDevolucion FROM  detallematerialdevuelto dtv 
                inner join detallematerial dt on dtv.idDetalleMaterial= dt.idDetalleMaterial 
                inner join estudiante e on e.idEstudiante=dtv.idEstudiante";    
        }
            $respuestaConsulta = $this->consulta($sql);
            // $row = $respuestaConsulta->fetch(PDO::FETCH_ASSOC);
            // if(!$row){
